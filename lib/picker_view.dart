...
class _MyDatePickerState extends State<MyDatePicker> {
  ...
  get _realDate => DateTime(now.year + yearIndex, monthIndex + 1, dayIndex + 1,
  ...
  void refreshScrollOffset() {
    ...
    yearScrollCtrl = FixedExtentScrollController(initialItem: yearIndex);
    monthScrollCtrl = FixedExtentScrollController(initialItem: monthIndex);
    dayScrollCtrl = FixedExtentScrollController(initialItem: dayIndex);
    hourScrollCtrl = FixedExtentScrollController(initialItem: hourIndex);
    minuteScrollCtrl = FixedExtentScrollController(initialItem: minuteIndex);
  }
  ...
  String? stringIndexByYear(int index) {
    if (index >= -100 && index <= 100) {
      ...
      return MyDate.isInRange(_nowDate, start: start, end: end)
          ? _year.toString() + '年'
          : null;
    }
    return null;
  ...
  changeDate(int _) {
    int days =
        MyDate.daysInMonth(DateTime(now.year + yearIndex, monthIndex + 1));
    DateTime date = DateTime(now.year + yearIndex, monthIndex + 1,
                    ...
                    return Container(
                      ...
                      child: Text(
                        ...
                        style: const TextStyle(fontSize: 18).copyWith(
                            color: widget.color ??
                                const Color(0xFF000046)), // theme.itemStyle,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          if (isShowHeader) renderHeader()??SizedBox(),
          renderSheet(context),
        ],
      ),
    );
